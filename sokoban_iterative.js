...
class node {
    constructor(data, pointer, nivel) {
      this.data = data;
...
function perception(data){
  ...
  function solver(queue, repeats, goals){
    ...
    while (queue.length!=0) {
        ...
        // else if (count==50000) { //Condicion de parada extra
        //   queue.shift();
        //   //break;
        // }
          ...
          if(present.nivel < limite ){
            repeats.push(present.data);
            var sense = perception(present.data);
            // console.log(sense);
            queue.shift();
            // ***************** ORDEN POR PROFUNDIDAD ***************** //
            for (var k = sense.length-1; k >= 0; k--) { //Recorre toda las percepciones recibidas
              ...
              queue.unshift(new node(value, present, present.nivel+1));
            }
